...
inputFile1 = LOAD 'hdfs:///user/srilakshmi/episodeIV_dialogues.txt' using PigStorage('\t') AS (name,line);
inputFile2 = LOAD 'hdfs:///user/srilakshmi/episodeV_dialogues.txt' using PigStorage('\t') AS (name,line);
inputFile3 = LOAD 'hdfs:///user/srilakshmi/episodeVI_dialogues.txt' using PigStorage('\t') AS (name,line);
...
STORE Sorted_Output INTO 'hdfs:///user/srilakshmi/Output_Characters';
